...
export function TransferTon() {
  ...
  return (
    <div>
      <div>
        <h3>Hello, {new URLSearchParams(window.location.search).get("username")}</h3>
        <h3>Wallet: {wallet}</h3>
        <h3>Balance: </h3>
        <h3>Transfer a deposit</h3>
        <div>
          <label>Pay now, get a discount later if you answer correctly and quickly.</label>
        </div>
        <div>
          <label><b>Price: {tonAmount} TONs.</b></label>
        </div>
        <button
          disabled={!connected}
          style={{ marginTop: 18 }}
